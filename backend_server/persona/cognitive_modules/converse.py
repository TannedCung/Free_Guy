...
def generate_agent_chat_summarize_ideas(
    init_persona: Persona, target_persona: Persona, retrieved: dict[str, list[ConceptNode]], curr_context: str
) -> str:
...
def generate_summarize_agent_relationship(
    init_persona: Persona, target_persona: Persona, retrieved: dict[str, list[ConceptNode]]
) -> str:
    all_embedding_keys: list[str] = []
    for key, val in retrieved.items():
        for node in val:
...
def generate_next_line(
    persona: Persona, interlocutor_desc: str, curr_convo: list[list[str]], summarized_idea: str
) -> str:
...
def load_history_via_whisper(personas: dict[str, Persona], whispers: list[list[str]]) -> None:
    for count, row in enumerate(whispers):
        ...
        persona.a_mem.add_thought(
            ...
            thought,
            keywords,
            thought_poignancy,  # type: ignore[arg-type]
