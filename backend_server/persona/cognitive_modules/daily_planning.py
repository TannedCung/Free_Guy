...
def generate_new_decomp_schedule(
    persona: Persona, inserted_act: str, inserted_act_dur: int, start_hour: int, end_hour: int
) -> list[list]:
    ...
    today_min_pass = (
        int(p.scratch.curr_time.hour) * 60  # type: ignore[union-attr]
        + int(p.scratch.curr_time.minute)  # type: ignore[union-attr]
        + 1
    )
