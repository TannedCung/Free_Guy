...
def _chat_react(
    maze: Maze, persona: Persona, focused_event: dict[str, Any], reaction_mode: str, personas: dict[str, Persona]
) -> None:
    ...
    for role, p in [("init", init_persona), ("target", target_persona)]:
        ...
        _create_react(
            ...
            convo,
            chatting_with_buffer,
            chatting_end_time,  # type: ignore[arg-type]
            act_pronunciatio,
            act_obj_description,
            act_obj_pronunciatio,
            act_obj_event,
...
def _wait_react(persona: Persona, reaction_mode: str) -> None:
    ...
    inserted_act_dur = (
        ...
        - (p.scratch.curr_time.minute + p.scratch.curr_time.hour * 60)  # type: ignore[union-attr]
        + 1
    )
